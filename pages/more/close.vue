	...
	<view class="page">
	   ...
	   <view class="my-frame-little" style="margin-top:24rpx;padding:10rpx 0;">
		...
		<view class="icon" @tap="send" hover-class="my-hover" :hover-stay-time="50" :hover-start-time="0">
	    	<image style="height:44px;width:44px" src="/static/i-close.png"></image>
	    </view>
	    <view class="my-header" style="color:#666;font-size:16px;">点击按钮，远程控制设备关机</view>
	</view>
...
<script>
	...
	export default {
		...
		methods:{
			send(){
				var that =this;
				var data = {"param_data":{"device_id":that.infor.device_id,"set_command":"turn_off_dev","set_value":""}};
				uni.request({url:this.url +"/device_set_person/",method:"POST",header:{"authorization": this.token},data:data,sslVerify:false,
				success: (res) => {
					// console.log(res);
					if (res.data.code == 1000) {
						uni.showModal({title:"远程关机",content:res.data.data.msg,showCancel:false});
					}else{
						var error = "错误码:"+res.statusCode;
						if(res.data.code){error=res.data.error};
						uni.showModal({title:"远程关机",content:"指令发送失败,"+error,showCancel:false});}
				},
